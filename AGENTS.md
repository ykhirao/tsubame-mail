...
Cloudflare の操作が要って**こちらでは進められない**ものは `docs/user-todo.md`。
...
- **実環境の値をコミットしない。** 実ドメイン・実メールアドレス・アカウント ID・
  リソース ID・メッセージ id は、コードにもドキュメントにも書かない
  （`example.com` / `example.net` などに伏せる）。実機で確かめた記録を残すときも同じで、
  伏せても記述の価値は変わらない。
