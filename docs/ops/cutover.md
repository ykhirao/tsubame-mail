...
- ルールとドメインの両方の付け替えを**短時間（数分以内）で**済ませれば、実害は
  ...
  **メールの喪失**は起きにくい。`tsubame` の受信ハンドラが例外を返せば Email Routing は一時失敗を返し、
  送信側の MTA が間隔を広げて再送してくる（精査 #24。実機で確認）。届かないのは、ハンドラが成功を返した後に
  キューの処理が 3 回失敗して DLQ に落ちたときで、その分は `docs/ops/operations.md` §2 の手順で積み直せる。
  それでも、重要なメールを送り主に再送してもらって確かめられるよう、**切り替えは業務の谷間（夜間・週末）に行う**。
