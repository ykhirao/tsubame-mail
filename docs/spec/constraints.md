# 受け入れた制約 — 精査の結果

**直さないと決めたもの**の置き場所。「これはバグではないのか」と思ったら、まずここを見る。
同じ議論を何度もやり直さないために残している。

観点（どう精査するか）は [セキュリティ観点](security.md)、
これからやることは [これからやること](todo.md)。ここには**残作業を書かない**。

- §1 はデプロイのたびに要る作業、**§2 が本体**（直していない・直しきれない約 60 件）、
  §3 は修正・審査・次の精査で必ず見る型。
- 番号「#n」は精査の通し番号で、コードのコメントや `security.md` から参照している。**次に振る番号は #148**。
  解決済みの指摘は消した。#1〜#120 の記録は `git show 84eb932:docs/spec/security-audit.md`、
  #121〜#126・#129 は対応のコミット（`git log --grep '#121'`）にある。
- 精査の定期的な輪は閉じた。**新しく見つけた指摘は、直してから §2 に残る制約だけを書く。**
  読んで疑っただけのものもここに置いてよい（「疑い」「場所」「どう確かめるか」の 3 行。番号は確かめてから振る）。
  確かめて成立しなければ、却下の理由を 1 行残してから消す——次の人が同じ道を歩かないように。
  走らせないと確かめられないものは [これからやること](todo.md) §2 へ。

## 1. デプロイ時に要る作業

**`0008_visibility_and_primary`（FR-4 / FR-11 / FR-19）は `scripts/deploy.sh` のマイグレーションで入るが、データ移行を含むので、デプロイ直後にオーナーが見えるメールが変わる。**
  owner の有効な API キーが対象にしているアドレスは、キーが動き続けるよう owner に**非表示**で割り当てる（受信箱には出ない。不要なら割り当てを外す）。
手順は `docs/ops/deployment.md` §5.3 と `docs/ops/operations.md` §8。要点:

- owner は割り当てたアドレスしか見なくなる。移行は「誰にも割り当てていないアドレス」だけを全 owner に write で割り当てる。
  **誰か 1 人にでも（read でも）割り当て済みのアドレスは owner に割り当てない**ので、そのメールはデプロイ直後に owner の受信箱・一覧・検索・通知・未読数から消える。
  読むには管理者モード、扱うには管理画面で自分に割り当て直す。
- 今のログイン用アドレス（`users.email`）は**確認済みの外部アドレス**に写す。今までどおりそれでログインできる。
- プライマリは、write で割り当てたメールボックス（エイリアス・アーカイブ済みを除く）のうち最初の 1 つを自動で選ぶ。
  write のメールボックスが無い利用者（read だけの member、割り当ての無い owner）はプライマリ無しになる。member / agent は本来必須なので、デプロイ後に管理画面で付ける（`⚠` が出る）。
- 既存の会話は変わらない。新しいテーブル `email_verifications`、列 `address_grants.hidden` / `sessions.admin_mode_until` / `users.external_email` / `external_verified_at` / `primary_address_id`。

#84 の Cookie の名前の変更、outbound の DLQ、`0004` は 2026-09-12 に本番へ反映済み。#25 の `0007`（`api_keys.parent_key_id`）は手作業が要らない。
新しい環境を作る手順は `docs/ops/deployment.md`。直した指摘がキュー・シークレット・マイグレーションを増やしたら、ここに書く。

## 2. 残っている制約

直していない、または直しきれないもの。**太字**は影響が大きいか、仕組みではなく運用で守っているもの。

### 受信

- スレッド詳細は最新の 200 件を返し、それより古いものは画面の「古いメッセージを読み込む」で遡る（#35）。From 偽装で 200 件積まれても最新は隠れない。
- 受信 `Date` は投入時刻の 1 年前〜10 分後の外なら投入時刻に落とすが、1 年以内の過去日は通る。新規スレッドならその日付で沈む（#19）。
- 同形異字（キリル文字の `а` など）は正規化しても同じにならず、拒否ルールをすり抜けうる（#9）。
- 表示用の `to_addr` は受信ヘッダの値のまま（ルールの照合はエンベロープで行う）（#14）。
- 山括弧の無い In-Reply-To / References（RFC 違反）はスレッドにつながらない（#72）。
- 受信の再配達の冪等化は「確認してから insert」なので、同じキューメッセージの同時再配達では Webhook の配信行が 2 本できうる。前回の添付 put の R2 孤児は残る（#77）。
- アドレスを消すと、そのアドレスのメールの生 MIME と添付を R2 からも消す（#147）。D1 の cascade で
  キーを知る手段が失われる前に集めておく。R2 の削除に失敗しても API は成功を返す（消し損ねた分は孤児として残るだけ）。
  ゴミ箱はメールを消さない（`status` を変えるだけ）ので、R2 に触るのはこの経路だけ。
- 解析できない・大きすぎるメール（placeholder）は、envelope の from が正規化できなければ差出人が空になる（#82）。
- アーカイブ済みのアドレス宛は、catch-all があればそこへ落ちる（#37）。
- **修正前に保存された行**は直っていない: ローカル部に裸のカンマがある `to_addr` / `cc_addr`（返信で宛先が割れる。#82）、
  `\` で終わる引用表示名（読めない。#39）、別ドメインを向いたエイリアス（`alias_target_id` に FK が無い。#61）。

- CF ブロックは「1 本目のヘッダが `by mx.cloudflare.net` の Received か」だけで見分ける。実機では Cloudflare が必ず一番上に足すので送信者は 1 本目を取れないが、
  Cloudflare を通らない経路（テストのひな形・将来の別の受け口）では偽の ARC で接ぎ木できる（#127）。
- 受信の接ぎ木は、Cloudflare が一番上に足したヘッダのまとまり（CF ブロック）の認証結果で DMARC pass か From のドメインの DKIM pass を要求する（#127）。
  DMARC も DKIM も無い小さなドメインの正規の返信は新しいスレッドになる（受け入れた副作用）。Cloudflare が ARC も Authentication-Results も付けなかったメールは未認証扱い。
- `spam_verdict` は CF ブロックの `X-CF-SpamH-Score` が 3 以上で `suspicious`、未満で `clean`、無ければ null（#128 / #140）。`spam` は出さない。
  実機で見えた値は普通のメールが 0〜1、GTUBE が 2、スパム語や IP 直書きの `.exe` リンクが 3〜4。
  それより露骨な見本は Cloudflare Email Sending が送信の時点で「スパムとして拒否」したので、外から届くメールに 5 以上が付くかは見ていない。1 通ずつの観測なので、実運用で値が集まったら `parse.ts` `spamVerdictFromScore` を見直す。
- CF ブロックの並びは、実機の 10 通（Cloudflare Email Sending 経由と Gmail から）で必ず `Received` → `ARC-Seal` → `ARC-Message-Signature` →
  `ARC-Authentication-Results` → `Received-SPF` → `Authentication-Results` → `X-CF-SpamH-Score` だった。
- 受け取りの Worker が例外を投げると、Email Routing は一時失敗を返し、送信側が再送する（#24。実機で確認）。Cloudflare Email Sending は
  約 26 秒・38 秒・71 秒・135 秒・605 秒と間隔を広げて、Gmail は 6 分後に送り直してきた。キューへの投入が落ち続ける間、同じメールが何度も来る。

### 送信

- 宛先ごとに送れた記録（`outbound_jobs.sent_recipients`）を持ち、再試行では未送信の宛先にだけ送る（#21 / #59）。1 通送った直後の記録の書き込みに失敗したときだけ、その宛先に再送しうる。
- 件名は 600 バイト（日本語で約 200 文字）までで、超えると 400。返信で「Re: 」を足して超えるときは元の件名を末尾から切る（#22）。
- 返信の引用は本文の上限に収まるよう末尾から切り、「（引用が長いため途中までです）」を添える。利用者が書いた部分は切らない（#115）。
- 非 ASCII の添付ファイル名は RFC 2231 で送るので、それを読まない古い MUA では化ける（#119）。
- 送信の直前に差出人の行が消えていれば送る（アドレスの削除はメッセージごと消える前提）（#67）。To が無く Cc / Bcc だけの送信は受けない（#80）。
- 送信に失敗した行は `failed` として残る（#90）。
- ドメインの送信の無効化（`POST /admin/domains/:id/sending {enabled:false}`）は tsubame の中で止めるだけで、Cloudflare の Email Sending と DNS はそのまま（#144）。
  送信 API は 409、無効化の前に積まれた分は `failed` になる。署名は画面のログインからだけ変えられ、API キーでは 403（#143）。
- `sent` は Cloudflare Email Sending が受け付けたという意味で、届いたことではない。binding の `send()` は成功を返したが、
  ダッシュボードの Activity log では「スパムとして拒否」や「配信失敗（550 5.1.1 Address does not exist）」になったメールを実機で見た。
  tsubame の画面には `sent` のまま出る。届いたかどうかは Cloudflare の Activity log でしか分からない。
- **スパムとして拒否されたメールやバウンスは、送信ドメインの評判（Bounce rate / Spam rejection rate）に数えられる。** 悪化すると
  Cloudflare が送信を止めうる。2026-09-12 の検証でスパムの見本と存在しない宛先に送ったら、test ドメインが「At Risk」になった。
  検証でスパムの見本を送るときや、存在しない宛先に送るときは、本番の送信ドメインを使わない。
- Cloudflare Email Sending も Email Routing も、998 文字を超えるヘッダ行（5,014 文字まで試した）を拒否も折り返しもせずそのまま通す（#34 / #66）。
  外の中継では、998 を超える行を「550 maximum allowed line length is 998 octets」や「Maximum line length exceeded (see RFC 5322 2.1.1)」で拒否する実例が公開されている。
  Gmail は Subject を 998 文字、ほかのヘッダを値 32KB までとしていて、超えると 552-5.3.4 で拒否する。送信側は 998 以内に収めているので、今は当たらない。

- LIKE に落ちる検索語は、前後の `%` を含めて 50 バイト（本番の D1 の上限。実機で確認）を超えると 400（#146）。drizzle のクエリ失敗はバインド値を落としてログに出す（`lib/logError.ts`）。

### 表示

- **受信 HTML の DOMParser を使う経路に自動テストが無い**（workerd に `DOMParser` が無く、vitest は正規表現の代用経路だけを見る）。mXSS 対策の収束ループ（#17）は
  Chrome の実機で確かめただけ。インライン SVG は表示されず、`<plaintext>` を含む本文は常に空。
- ゴミ箱のメッセージの添付と生 MIME は `?includeTrash=true` を付けたときだけ読める（#116）。本体と同じく、付ければ読めるのは意図どおり。
- 本文のリンクは新しいタブで開く（#141）。iframe の sandbox に `allow-popups allow-popups-to-escape-sandbox` を足し、`MessageHtml.tsx` `rewriteLinks` が
  http(s) / mailto 以外の `href` を落として `target="_blank" rel="noopener noreferrer"` を付ける。開いた先は普通のタブなので、リンク先そのものの危険はブラウザ任せ。
  この書き換えは DOMParser の経路にしか無く、vitest は `safeLinkHref` だけを見る（上の「自動テストが無い」と同じ）。
- CSP に `sandbox` は付けていない（添付のダウンロードを止めるブラウザがあるため）（#6）。
- 本文のリンクを押した瞬間（pointerdown）に、Chrome はリンク先の DNS 解決と TCP 接続を始める。押したまま外へずらしてクリックを取り消しても接続は起きる。
  表示しただけ・ホバーしただけでは起きない（Chrome 152 で確認）。漏れるのは IP と「押しかけた」ことで、画像の表示を許可したときより狭い。

### 認証・キー

- キーで発行したキーは `api_keys.parent_key_id` に親を持ち、親の失効・差し替え・持ち主の削除・パスワード変更・無効化で、他の利用者向けの子孫も失効する（#25 / #142）。無効化では本人のキーは残す（再有効化で戻る）。`0007` より前に発行された子キーは親を持たないので連鎖しない。
- 範囲を絞ったキーが `/me/api-keys` から失効できるのは、そのキー自身とその子孫だけ（#145。管理 API からは失効できない）。
- 範囲を絞った admin キーでも管理の一覧 GET（他人のキーの prefix、Webhook の URL など）は読める。変更系と監査ログだけ 403（#129 の設計判断）。
- `PATCH /me`（表示名）はスコープを問わず通り、`GET /me` は read スコープの無いキーにも割り当てアドレス（署名を含む）を返す。自分の情報なので受け入れている。
- **Rate Limiting binding が本番で発火しない（#147。2026-09-13 に実機で確認）。** ログイン（20 回 / 60 秒）を
  同じ IP・同じアドレス・同じ colo（NRT）で 30 回並列に叩いても全部 401 のままで、429 が 1 回も出ない。
  bootstrap（IP 単位）も 25 回で出ない。`wrangler` は 4.130.0（必要な 4.36.0 以上）、設定も
  `namespace_id` / `simple.period: 60` ともに公式の要件どおり。同じ症状が Cloudflare Community に
  報告されているが（`limit:1, period:10` でも出ない）、**回答が付かないまま 15 日で自動クローズ**されている。
  送信側（100 回 / 60 秒）も同じ binding なので、同じく効いていないとみておく。
  **総当たりの上限は今このアプリには無い**。当面はパスワードの強度と、失敗を数える監視で補う。
  カウンタは colo 単位なので、仮に効いても実効上限は「limit × 散った colo 数」になる。
- 無効な API キーを繰り返し送る相手は、IP 単位で数えて一定回数を超えたらしばらく 401 を即返しする（#147 の穴埋め）。
  10 分の窓で 20 回を超えたら 15 分。**キー自体は 32 バイトの乱数**なので総当たりで当てられる心配は無く、
  止めたいのは無効なキーで D1 を引き続ける負荷の方。
  - **正しいキーは止めている最中でも必ず通し、通った時点で記録ごと消す。** IP は NAT・社内網・CI で
    共有されるので、同居する誰かの失敗で正規の利用者を締め出さない。止めている相手も 1 回は照合する
    （`api_keys.key_hash` の索引 1 本なので、通す判断のための負荷は小さい）。
  - 止めている間は数え直さないので、期限が延び続けて実質解けなくなることはない。
  - IP は SHA-256 で保存する（`auth_failures.ip_hash`）。失敗しただけの相手の所在を生では残さない。
  - 記録に失敗しても認証の結果は変えない。数える側の不具合で正しいキーを締め出す方が害が大きい。
  - 窓も期限も切れた行は 5 分ごとの cron（`pruneAuthFailures`）が消す。
- ログインのレート制限: `email` 単独の鍵は、他人のアドレスを 1 分に 20 回叩けばその人のログインを止められる。`ip` 単独の鍵は NAT 配下の全員で分け合う。
  成功したログインもバケットを消費する（#52）。
- Fetch Metadata の検査は、`Sec-Fetch-Site` も `Origin` も無い要求（非ブラウザ・古いブラウザ）を通す（#76）。
- owner が管理 API で自分のパスワードを変えると、自分にも強制変更が付く（`PATCH /me` で解除できる）（#98）。
- vitest 用の固定の `INTERNAL_SECRET` はリポジトリにあり、本番の拒否リストには入っていない（#48）。
- ユーザー管理の変更系（作成・更新・削除・grants）は API キーでは 403。自動化するなら画面ログインのセッションが要る（#121）。
- 管理 API の変更系（webhook・ルール・ドメイン・アドレス）は、addressIds を絞ったキーでは 403。addressIds が全部のキーは期限付きでも通るので、
  そのキーで作ったルールや webhook はキーの期限が切れた後も残る（#129）。「絞った」の判定は `Principal.keyRestricted`（キーが `address_ids` を持つか）で、
  owner の割り当てが `"all"` でなくなった今も同じ範囲を守る。

### 見る範囲・ログイン識別子（FR-4 / FR-11 / FR-19）

- **旧 `users.email` 列は残っている**（NOT NULL・一意）。外部アドレスの写しで、外部アドレスの無い利用者には `<id>@users.invalid` が入る。
  ログイン・一意性の判定には使わない。NOT NULL を外すにはテーブルの作り直しが要り、D1 では子（セッション・キー・割り当て）の cascade 削除が走るので残している（`schema.ts` のコメント）。
- **管理者モードの期限は `sessions.admin_mode_until` に置いたままで、切れても消さない**。効くかどうかは毎リクエスト `resolvePrincipal` が時刻で判定する。
  「出る」を押さずに閉じると、行には過去の時刻が残る（害は無いが、監査ログに `admin_mode.exit` は出ない。切れたことを知るには `enter` の `meta.until` を見る）。
  延長の API は無く、続けるには入り直す（`enter` がもう 1 行残る）。
- 管理者モードで**読んだ**メールは記録しない（入った・出ただけ）。誰のメールを開いたかは監査ログから分からない。
- 管理者モードで `GET /addresses` / `GET /me` は全アドレスを返す。読めるだけのアドレスは `level: "read"` で出るので、画面はそれで書けないことを示す。
  既読・スター・ゴミ箱は API が 403 で止める（`canModify`）。自動既読は画面側でも `ownAddressIds` を見て抑える。
- **外部アドレスの確認コードは 6 桁を塩なしの SHA-256 で置く**（`email_verifications.code_hash`。`hashToken` はセッション・API キーと同じ関数）。
  100 万通りしかないので、DB が漏れれば行ごとに一瞬で逆算できる。オンラインの総当たりは 5 回の試行上限と 30 分の期限で止めている。
  コードは 30 分で切れ、失敗 5 回か期限切れで行が消え、送り直しが要る。
- 確認メールは、登録・再送とも前回から 60 秒空け、1 利用者あたり 1 時間に 5 通まで（監査ログの `user.external_email.set` / `.resend` を数える）。
  宛先を変えても同じ枠を使うので、第三者への踏み台としては 1 時間 5 通が上限。
- 確認メールの差出人は本人のプライマリ → owner のプライマリ → 送信できる最古のメールボックスの順で選ぶ。送信できるドメインが無ければ `sent: false` と理由を返し、外部アドレスは未確認のまま登録される。
  プライマリの無い利用者（最初の owner）はその未確認のアドレスでログインできるが、プライマリが付いた瞬間（最初のメールボックスを作った時）から、確認するまで外部アドレスでは入れなくなる。
- owner が他人の外部アドレスを登録できる（`PUT /admin/users/:id/external-email`）。確認コードは相手のアドレスに届くので owner はログインを乗っ取れないが、相手のログイン先を未確認に戻して止めることはできる（監査ログに残る）。
- 非表示（`address_grants.hidden`）は監査ログに残らない（見え方の設定）。`PATCH /addresses/:id/hidden` は API キーでも通る。
- `POST /admin/addresses` の `assignToMe` と最初のメールボックスの自動プライマリは `address.create` の `meta` に出ない。誰に割り当てたかは `GET /admin/addresses/:id/viewers` で見る。
- `PATCH /admin/users/:id` の `primaryAddressId` は `user.update` として残るが、`meta` に新しいプライマリは出ない。

### プッシュ通知

- 端末の購読はそのセッションと同じ寿命。セッションが切れる（30 日）かログアウトすると、その端末には届かなくなる（#130）。
- 同じ endpoint を別の利用者が登録すると、購読を持つブラウザを持ち主として取り替える（#130）。endpoint を知られると通知を横取りはできないが（鍵が違うので読めない）、止められる。
- 一時的に失敗した端末にだけ、キューの再試行（最大 3 回）で送り直す。成功した端末には二度送らない（#131）。再送の本文は通常の受信通知で作り直す（最初が「新着 N 件」のまとめでも）。
- 恒久的な失敗（4xx）が 3 回続いた端末は無効にする。どの 4xx を恒久とみなすかは push サービスの実挙動を見ていない（#131）。
- 利用者あたり端末 10 台・通知ルール 50 件まで（#133）。

### Webhook

- `pending` のまま止まった配信（手動再送の claim の後に無効化した #69、キューのメッセージを失った #42）は、再試行の予定（無ければ作成）から 30 分経てば手動で再送できる。同じ試行番号の POST は `runDelivery` が試行番号を先に取った 1 回だけ。30 分は処理中の配信を止まったと誤って扱わないための待ち。
- DNS リバインディングは Workers から防げない（#12）。

### ドメイン・Cloudflare

- subdomain モードの切断は、名前の完全一致と DKIM のホスト名だけを消す。従来消していた `*.<name>` のメール用レコードは残る（消さない側に倒した）（#60）。
- `verifyDomain` / `previewDomain` のページ上限（最大 41 往復）はそのまま（#93）。Cloudflare API の上限は利用者ごとに 5 分で 1,200 回で、
  応答の `ratelimit` / `ratelimit-policy` ヘッダで確かめた（1 回の検証で最大 3% ほど使う）。上限はダッシュボード・API キー・トークンを合わせて数えられ、
  超えると以後 5 分間は全部の呼び出しが 429 になり、`Retry-After` が付く（Cloudflare の文書）。`cloudflare-api.ts` は 429 を特別扱いせず、ふつうの失敗として返す。
- `CF_ACCOUNT_ID` が未設定だとドメイン接続が動かない（必須として文書化）（#94）。
- 監査ログは 400 日で消える（#95。`docs/ops/audit-log.md`）。それより長く残すには書き出して別に保管する。

### 運用・CI

- `npm ci` は install script を持つ依存（`workerd`・`esbuild`・`core-js-pure`・`fsevents`）を全部実行する。絞っていない（#96）。
- `0004` の `'rebuild'` はバッチ無しの全件再構築で、手順で扱っている（#112）。
- ローカル開発（`http://localhost` / `http://127.0.0.1`）には Safari ではログインできない。Safari 26.6 は `Secure` 付きの Cookie を http では保存しない
  （`__Host-` に限らない）。Chrome・Firefox・Brave は保存する（#84）。

## 3. 再発しやすい型

これまでの輪で繰り返し出た形。修正・審査・次の精査で必ず見る。

- **保存形式の往復が冪等でない**。カンマ区切りで保存するアドレス列は、表示名（#39）でもローカル部（#82）でも同じ穴が開いた。区切り文字・引用・エスケープを持つ形式を増やしたら、format → parse → format の往復をテストで固定する。
- **門が 1 経路にしか入っていない**。新規送信だけに入れた上限が返信に無い（#89 → #115）、通常の受信だけ正規化して placeholder を忘れた（#82）、
  自分のルータだけ検査して同じ経路の他ルータが素通り（#13 / #31 / #75）。直したら `grep` で同じ入力を受ける経路を全部洗う。
- **キューの at-least-once**。再配達で副作用が二重になる（#21 / #86 / #118）か、重複判定で捨てられて続きが永久に走らない（#56 / #77）。再配達は即時に来る前提で、期限付きの回収が動くかも見る。
- **D1 の上限**。バインド変数 100 個（#32 / #57 / #88。id の列は `policy.ts` の `jsonIdsIn` で JSON 1 本にする）、1 行 2MB（#89 / #115）。どちらも vitest では再現しにくい。
- **キーから、キーより強い資格情報・広い範囲を得られる**。子キーの発行（#25 / #58）、期限のオーバーフロー（#83）、ユーザー管理（#121）、
  管理 API の Webhook・ルール（#129）。キーの addressIds と期限は、そのキーで作れるもの・届くもの全部を縛るか確かめる。
- **Cloudflare のゾーン単位の資源をドメイン単位で扱う**。catch-all（#85 / #117）、DNS レコード（#18 / #60）。消す操作は「迷ったら消さず 409」。
- **API に上限・カーソルを付けたのに、呼ぶ UI が 1 ページしか読まない**（#32 / #65 / #81）。
- **テストが本番と違う経路を通る**。vitest の `new Request` は workerd の実 HTTP とボディの扱いが違う（#55）。実装者が書いたテストが誤った挙動を期待値にしていた（#117 の第 2 波）。
- **設定値の検証漏れが非同期処理を全部止める**。保存時に検査しなかった値（`quiet.tz`）が、キューのコンシューマで毎回例外になり DLQ へ行く（#132）。保存時に検査し、読む側も例外で止まらない値に倒す。
- **Service Worker はクラシックスクリプト**。画面と共有するモジュールを import すると共通チャンクへの import が混ざり、登録ごと失敗する（ビルドで検査している）。
- **ブラウザ実機でしか分からない**。parse → serialize → reparse が冪等でない mXSS、iframe の投機接続（#17）。
