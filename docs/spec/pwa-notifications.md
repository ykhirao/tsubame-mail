...
| 2 | アドレスへの割り当てが無い | 対象外。**owner も同じ**（割り当てが要る。キャッチオールの受け皿も同じ）。管理者モードは通知に影響しない | `not_assigned` |
...
- キャッチオールの受け皿は**割り当てた人にだけ**通知する（owner でも割り当てが要る。FR-16 / FR-19）。割り当てがあれば既定で「すべての新着」。
  キャッチオールは画面でラベルを付けて見分けられるようにする。
- キャッチオールは**専用のスイッチ「キャッチオールを通知する」**でまとめてオン・オフできる。
  通知ルールはこのスイッチより先に効く（「必ず通知」に一致すれば、スイッチがオフでも通知する）。
- 管理者モードは通知・未読数・通知設定の対象を広げない（`services/notify/load.ts` `eligibleUserIds`、`prefs.ts` `countUnread`、`api/v1/notifications.ts` `loadVisibleAddresses`）。
  非表示（`address_grants.hidden`）も通知を変えない。
