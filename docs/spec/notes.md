...
| `解決` / `対応済` | 確かめて成立し、その場で直した。どこで直したかを 1 行で残す |
...
- `解決` **精査 #5 の「回り続ける」は `max_retries: 3` で止まる。** 精査 #20。`wrangler.jsonc` に受信・送信の DLQ を足し、パース例外は `inbound.ts` `parseErrorPlaceholder` で行を残すようにした。DLQ に落ちた分の見方は `docs/ops/operations.md` §2。
